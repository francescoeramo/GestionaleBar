...
def _build_order(order_id: int, db: sqlite3.Connection) -> dict:
    ...
    items = dict_rows(db.execute(
        "SELECT * FROM order_items WHERE order_id=? ORDER BY created_at", (order_id,)
    ).fetchall())
    for item in items:
        item["modifications"] = dict_rows(db.execute(
            "SELECT * FROM order_item_modifications WHERE order_item_id=?", (item["id"],)
        ).fetchall())
    order["items"] = items
    return order
...
def open_order(data: OrderCreate, db: sqlite3.Connection = Depends(get_db)):
    if data.table_id:
        existing = db.execute(
            "SELECT id FROM orders WHERE table_id=? AND status='open'", (data.table_id,)
        ).fetchone()
        if existing:
    ...
    db.commit()
    return _build_order(order_id, db)
...
def close_order(id: int, data: OrderClose, db: sqlite3.Connection = Depends(get_db)):
    ...
    if round(paid, 2) < round(total_net, 2):
        raise HTTPException(
            400, f"Pagamento insufficiente: pagato {paid:.2f}€, dovuto {total_net:.2f}€"
        )
